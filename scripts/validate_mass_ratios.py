#!/usr/bin/env python3
"""
Validate best model configuration across multiple mass ratios.

Tests the optimal kernel-warping configuration at various mass ratios to:
1. Understand performance at training mass ratios (q=0.1, 0.8)
2. Assess interpolation quality (q=0.3, 0.5, 0.7)
3. Evaluate extrapolation limits (q=0.05, 0.9)

Usage: python validate_mass_ratios.py <mass_ratio>
"""
...
if len(sys.argv) < 2:
    print("Usage: python validate_mass_ratios.py <mass_ratio>")
    sys.exit(1)
...
print("=" * 70)
print(f"Mass Ratio Validation: q = {q}")
print("=" * 70)
print(f"Configuration: Matérn(ν={NU}) + Chirp(α={ALPHA})")
print(f"Training iterations: {N_ITER}")
print("=" * 70)

# Determine if this is a training, interpolation, or extrapolation test
if abs(q - 0.1) < 0.02 or abs(q - 0.8) < 0.02:
    test_type = "TRAINING"
elif q > 0.1 and q < 0.8:
...
# Save to file
output_file = 'mass_ratio_validation_results.txt'

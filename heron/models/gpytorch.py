...
class ExactGPModelMatern(gpytorch.models.ExactGP):
    """
    GP model with Matérn kernels instead of RBF.

    Matérn kernels can handle non-smooth features better than RBF,
    making them more suitable for gravitational waveforms with sharp peaks
    and exponential ringdown decay.

    ν parameter controls smoothness:
    - ν=1.5: once-differentiable (more flexible, handles sharp features)
    - ν=2.5: twice-differentiable (smoother, default)
    """
    def __init__(
        self, train_x, train_y, likelihood=gpytorch.likelihoods.GaussianLikelihood(),
        nu=2.5
        ...
        # Use Matérn kernels for both dimensions
        # Default ν=2.5 for twice-differentiable functions
        self.covar_module = gpytorch.kernels.ScaleKernel(
            gpytorch.kernels.MaternKernel(
                nu=nu,
                active_dims=[0],
                lengthscale_constraint=gpytorch.constraints.GreaterThan(0.01),
            )
            * gpytorch.kernels.MaternKernel(
                nu=nu,
                active_dims=[1],
    ...
    def time_domain(self, parameters, times=None):
        ...
        for polarisation in ("plus", "cross"):
            ...
            output.waveforms[polarisation] = Waveform(
                ...
                times=times,
                covariance_gpu=observed_pred.covariance_matrix,  # Keep on GPU for lazy transfer

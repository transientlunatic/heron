...
class ExactGPModelKeOps(gpytorch.models.ExactGP):
    def __init__(self, train_x, train_y, likelihood = gpytorch.likelihoods.GaussianLikelihood()):#noise_constraint=gpytorch.constraints.LessThan(3))):
        super(ExactGPModelKeOps, self).__init__(train_x, train_y, likelihood)
        self.mean_module = gpytorch.means.ZeroMean()
        self.covar_module = gpytorch.kernels.ScaleKernel(
            gpytorch.kernels.keops.RBFKernel(active_dims=[0],
...
class HeronNonSpinningApproximant(WaveformSurrogate, GPyTorchSurrogate):
    ...
    def _make_evaluation_manifold(self,
                                  parameter_min, parameter_max, parameter_n,
                                  time_min, time_max, time_n):
        test_mass_ratio = torch.linspace(parameter_min, parameter_max, parameter_n, dtype=torch.float32).cuda()
        test_times = torch.linspace(time_min, time_max, time_n, dtype=torch.float32).cuda()
    ...
    def time_domain_manifold(self, parameters):
        ...
        t = parameters["time"]
        points = self._make_evaluation_manifold(a['lower'], a['upper'], a['number'],
    ...
    def time_domain(self, parameters):
        ...
        points = torch.vstack([
            ...
            torch.linspace(t['lower'], t['upper'], t['number'], dtype=torch.float32).cuda()
            ]).T

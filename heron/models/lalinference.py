...
class IMRPhenomPv2(Model):
    ...
    def time_domain_waveform(self, p, times):
        ...
        hp, hx = lalsimulation.SimInspiralChooseTDWaveform(
            m1, m2, 0, 0, 0, 0, 0, 0, p['distance'], p['inclination'], 0, 0, 0, 0, dt, 20., 5., {}, approximant
        )
        sample_times = hp.epoch.gpsNanoSeconds * 1e-9 + hp.epoch.gpsSeconds + torch.linspace(0, (len(hp.data.data))*hp.deltaT, len(hp.data.data), dtype=torch.float64)
        idx = (sample_times >= float(times[0])) & (sample_times <= float(times[-1])+dt/2)
        ...
        detection = Timeseries(data=torch.tensor(waveform_mean, device=self.device), times=torch.tensor(times, device=self.device),
                               detector=p['detector'])

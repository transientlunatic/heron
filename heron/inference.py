...
def inference(settings):
    ...
    report = otter.Otter(os.path.join(settings['report']['location'],
                                      f"{settings['name']}.html"),
                         author="Heron",
    ...
    times = torch.linspace(settings['data']['start time'], settings['data']['start time'] + duration, floor(duration*srate))
    ...
    if "injection" in settings:
        ...
        psd = heron.injection.psd_from_lalinference(settings['noise model']['name'],
                                                    frequencies=heron.injection.frequencies_from_times(times))
        noise = heron.injection.create_noise_series(psd, times)
        signal = models[settings['injection model']].time_domain_waveform(times=times, p=settings['injection'])
    ...
    heron_likelihood = CUDATimedomainLikelihood(
        models[settings['waveform']['model']], times=times, data=detection, detector_prefix="L1", psd=psd
    )

    click.echo(f"Created likelihood on {heron_likelihood.device}")

    nessai_model = HeronSampler(heron_likelihood,
                                settings['priors'],
                                settings['injection'],
